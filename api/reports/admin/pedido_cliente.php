...
if ($pedido->setIdPedido($_GET['idPedido'])) {
    ...
    if ($rowOne = $pedido->readOneFactura()) {
        ...
        $pdf->ln(12);
        $pdf->setFont('Arial', 'B', 10);
        $pdf->cell(0, 0, 'Fecha en que se hizo: ' . $rowOne['fecha_registro'] . '   Forma de pago: ' . $rowOne['forma_pago_pedido'], 0, 1, 'C');
        // Se agrega un salto de línea
        $pdf->ln(10);
        // Ajustar los márgenes a 1.5 cm
        $pdf->setMargins(15, 0, 15);
        // Se verifica si existen registros para mostrar, de lo contrario se imprime un mensaje.
        if ($dataP = $pedido->readOneFactura()) {
            ...
            foreach ($dataP as $rowP) {
                ...
                $pdf->cell(39.12, 10, $pdf->encodeString($rowP['fecha_registro']), 'TB', 0, 'C');
                $pdf->cell(29.34, 10, $pdf->encodeString($rowP['cantidad_producto']), 'TB', 0, 'C');
